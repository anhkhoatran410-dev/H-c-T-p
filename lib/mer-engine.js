...
async function ask(model,prompt,image,timeout,maxOutputTokens){
  ...
  try{
    const r=await fetch('https://generativelanguage.googleapis.com/v1beta/models/'+model+':generateContent',{method:'POST',headers,body:JSON.stringify({contents:[{role:'user',parts}],generationConfig:{maxOutputTokens,thinkingConfig:{thinkingLevel:'high'}}}),signal:AbortSignal.timeout(timeout)});
