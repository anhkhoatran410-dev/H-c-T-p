...
const DEFAULT_GEMINI_MODELS = ['gemini-3.8-flash', 'gemini-3.7-flash', 'gemini-3.6-flash', 'gemini-3.5-flash-lite'];
const MAX_OUTPUT_TOKENS = 12000;
const VERIFY_RETRIES = 1;
function geminiModels() {
  const configured = cleanKey(process.env.GEMINI_MODEL);
  const extra = String(process.env.GEMINI_FALLBACK_MODELS || '').split(',').map(cleanKey).filter(Boolean);
  return [...new Set([configured, ...extra, ...DEFAULT_GEMINI_MODELS].filter(Boolean))];
}
function routerModel() {
  return cleanKey(process.env.GEMINI_ROUTER_MODEL || process.env.GEMINI_MODEL || DEFAULT_GEMINI_MODELS[0]);
}
...
async function analyzeRoute({ message, subject, image }) { const apiEntry = await acquireAiKey('GEMINI'); if (!apiEntry) return null; const api = apiEntry.key; const parts = [{ text: `Bạn là ROUTER của STUDY TH. Hãy phân tích bài toán trước khi Solver giải. Chỉ quyết định xem Code Execution có thực sự hữu ích hay không.\n\nQUY TẮC:\n- needs_code=true nếu bài có tính toán số học lớn, đếm tổ hợp/phân hoạch phức tạp, kiểm tra số dư/số nguyên tố, duyệt tập hữu hạn lớn, hoặc cần kiểm tra thuật toán/giả thuyết bằng Python.\n- needs_code=false nếu bài chủ yếu là chứng minh logic, hình học thuần túy, biến đổi đại số hoặc lập luận mà code không giúp đáng kể.\n- Không chọn true chỉ vì bài được gắn HARD/CHALLENGE.\n- problem_type là nhãn ngắn gọn như geometry, combinatorics, number_theory, algebra, proof, arithmetic, other.\n- difficulty là EASY, MEDIUM, HARD hoặc CHALLENGE.\n- reason giải thích ngắn gọn vì sao chọn nhánh.\n\nMÔN: ${subject || 'Toán'}\nĐỀ:\n${message}` }]; const img = imagePart(image); if (img) parts.push(img); try { const r = await fetch('https://generativelanguage.googleapis.com/v1beta/models/' + encodeURIComponent(routerModel()) + ':generateContent', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-goog-api-key': api }, body: JSON.stringify({ contents: [{ role: 'user', parts }], generationConfig: { maxOutputTokens: 700, responseMimeType: 'application/json', responseSchema: ROUTER_SCHEMA, thinkingConfig: { thinkingLevel: 'medium' } } }), signal: AbortSignal.timeout(6500) }); const raw = await r.text(); let d = {}; try { d = raw ? JSON.parse(raw) : {}; } catch (_) {} if (!r.ok) { await reportAiFailure('GEMINI', apiEntry.id, { status: r.status, providerMessage: d?.error?.message }); return null; } await reportAiSuccess('GEMINI', apiEntry.id); const txt = d?.candidates?.[0]?.content?.parts?.filter(p => p.text).map(p => p.text).join('').trim(); const parsed = parseJson(txt); if (!parsed || typeof parsed.needs_code !== 'boolean' || !parsed.problem_type || !parsed.difficulty) return null; return { needs_code: parsed.needs_code, reason: String(parsed.reason || ''), problem_type: String(parsed.problem_type), difficulty: String(parsed.difficulty).toUpperCase(), model: routerModel(), used: true }; } catch (_) { return null; } }
async function gemini({ message, subject, history, image, cas, model, deep = true, maxOutputTokens = MAX_OUTPUT_TOKENS, timeout = 30000, codeExecutionTools = false, thinkingLevel = null }) { const apiEntry = await acquireAiKey('GEMINI'); if (!apiEntry) { const e = new Error('Không còn Gemini key khả dụng.'); e.code='AI_CONFIG_MISSING'; throw e; } const api = apiEntry.key; const mode = subjectMode(subject, message); const olympiad = isOlympiadMath(subject, message); const parts = [{ text: solverPrompt(subject, mode, cas, message, history, olympiad, codeExecutionTools) }]; const img = imagePart(image); if (img) parts.push(img); const started = Date.now(); const body = { contents: [{ role: 'user', parts }], generationConfig: { maxOutputTokens: deep ? Math.min(maxOutputTokens, 9000) : Math.min(maxOutputTokens, 6000), ...(model !== 'gemini-3.5-flash-lite' ? { thinkingConfig: { thinkingLevel: thinkingLevel || (deep ? 'high' : 'low') } } : {}) } }; if (codeExecutionTools === true) body.tools = [{ codeExecution: {} }]; try { const r = await fetch('https://generativelanguage.googleapis.com/v1beta/models/' + model + ':generateContent', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-goog-api-key': api }, body: JSON.stringify(body), signal: AbortSignal.timeout(Math.min(timeout, 20000)) }); const raw = await r.text(); let d = {}; try { d = raw ? JSON.parse(raw) : {}; } catch (_) {} if (!r.ok) { const e = Object.assign(new Error(providerError(r.status, d?.error?.message)), { status: r.status, providerMessage: d?.error?.message }); await reportAiFailure('GEMINI', apiEntry.id, e); const retryAfter = Number(r.headers.get('retry-after')); if (Number.isFinite(retryAfter)) e.retryAfterMs = retryAfter * 1000; throw e; } const c = d?.candidates?.[0]; const answer = c?.content?.parts?.filter(p => p.text).map(p => p.text).join('').trim(); if (answer) { await reportAiSuccess('GEMINI', apiEntry.id); return { answer, model, finishReason: c?.finishReason || null, providerLatencyMs: Date.now() - started, codeExecutionUsed: Boolean(c?.content?.parts?.some(p => p.executableCode || p.codeExecutionResult)), codeExecutionRequested: codeExecutionTools === true }; } const emptyError = new Error('AI trả về rỗng.'); await reportAiFailure('GEMINI', apiEntry.id, emptyError); throw emptyError; } catch (e) { if (transient(e?.status, e?.providerMessage || e?.message, e)) e.isTransient = true; throw e; } }
async function solveWithFallback(args) {
  ...
  const models = tier.tier==='deep' ? geminiModels().slice(0,2) : geminiModels().slice(0,1);
  let last = null;
...
async function verify({ message, subject, image, cas, candidate }) { const apiEntry = candidate ? await acquireAiKey('GEMINI') : null; const api = apiEntry?.key || null; if (!api || !candidate) return null; const prompt = `Bạn là GIÁM KHẢO ĐỘC LẬP của STUDY TH. Không được tin lời giải ứng viên. Hãy tự giải bài từ đề gốc trước, sau đó đối chiếu.\n\nKIỂM TRA:\n1. Xác định chính xác yêu cầu, điều kiện và dữ kiện.\n2. Tự tìm đáp án/kết luận độc lập.\n3. Kiểm tra biến đổi, suy luận, điều kiện, mất/thêm nghiệm và tính toán.\n4. Thế ngược/kiểm tra trực tiếp khi phù hợp.\n5. Với Olympic/AIME, nếu hai cách khác nhau phải tìm điểm phân kỳ; không chấp nhận lời giải chỉ vì có vẻ hợp lý.\n6. Nếu có phản ví dụ hoặc suy luận không hợp lệ dẫn tới kết luận sai: FAIL.\n7. Nếu hai cách độc lập khớp và điều kiện quan trọng được kiểm tra: PASS.\n8. Chỉ UNCERTAIN khi dữ liệu thực sự không đủ.\n\nTrả JSON duy nhất:\n{"verdict":"PASS|FAIL|UNCERTAIN","expected_answer":"...","issues":["..."],"tests":[{"test":"...","result":"PASS|FAIL|NOT_APPLICABLE","note":"..."}],"repair_hint":"..."}\n\nMÔN: ${subject || 'Toán'}\nĐỀ:\n${message}\nCAS/WOLFRAM:\n${cas || 'Không có'}\nLỜI GIẢI ỨNG VIÊN:\n${candidate.answer}`; const parts = [{ text: prompt }]; const img = imagePart(image); if (img) parts.push(img); const models = geminiModels().slice(0,3); for (const model of models) { for (let attempt = 0; attempt < VERIFY_RETRIES; attempt++) { try { const r = await fetch('https://generativelanguage.googleapis.com/v1beta/models/' + model + ':generateContent', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-goog-api-key': api }, body: JSON.stringify({ contents: [{ role: 'user', parts }], generationConfig: { maxOutputTokens: 4200, responseMimeType: 'application/json', thinkingConfig: { thinkingLevel: 'high' } } }), signal: AbortSignal.timeout(11000) }); const raw = await r.text(); if (!r.ok) { await reportAiFailure('GEMINI', apiEntry.id, { status: r.status }); if (transient(r.status, '', { status: r.status }) && attempt < VERIFY_RETRIES - 1) { await sleep(1000); continue; } break; } let d = {}; try { d = raw ? JSON.parse(raw) : {}; } catch (_) {} const txt = d?.candidates?.[0]?.content?.parts?.filter(p => p.text).map(p => p.text).join('').trim(); const parsed = parseJson(txt); if (parsed && ['PASS', 'FAIL', 'UNCERTAIN'].includes(String(parsed.verdict || '').toUpperCase())) { await reportAiSuccess('GEMINI', apiEntry.id); parsed.verdict = String(parsed.verdict).toUpperCase(); return parsed; } } catch (e) { if (transient(e?.status, e?.message, e) && attempt < VERIFY_RETRIES - 1) { await sleep(1000); continue; } } } } return null; }
