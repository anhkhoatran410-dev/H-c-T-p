...
async function runExperts(prompt,image,timeout,tokens,onStage=null){
  ...
  try{if(typeof onStage==='function')await onStage('experts_started',{models:MODELS})}catch{}
...
export async function merSolve({message,subject,history,image,olympiad=false,onStage=null,verifiedByTool=false}){
  ...
  let expertRun;
  try{
    expertRun=await runExperts(primary,image,EXPERT_TIMEOUT,3600,onStage);
  }catch(e){
    throw e;
  }
  ...
  const allowEarlyExit=Boolean(verifiedByTool&&!proofLike(subject,message,olympiad));
  ...
  if(allowEarlyExit && experts.length>=2 && bestAgreement===1 && !repairable(expertBest.answer)){
  ...
  let out=expertBest,reviewLatencyMs=0,reviewFallback=false;
  await stage('review_started',{reason:experts.length<2?'single_expert':'experts_disagree'});
  try{
    const rs=await runExperts(review,image,REVIEW_TIMEOUT,4800,onStage);
    out=rs.experts[0];
  ...
  if(repairable(out.answer)){
    try{
      ...
      const rr=await runExperts(repairPrompt,image,7000,4200,onStage);
  ...
  return {
    ...
    used:true,
    reviewSkipped:false,
    finalized:!reviewFallback,
    degraded:reviewFallback,
    timeoutFallback:reviewFallback,
    agreementScore:Number(bestAgreement.toFixed(3)),
    latencyMs:Date.now()-started,
    expertLatencyMs:expertRun.latencyMs,
    reviewLatencyMs
