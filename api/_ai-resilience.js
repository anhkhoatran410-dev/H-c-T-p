...
const FAILURE_THRESHOLD=3,COOLDOWN_MS=60000,PROBE_MS=15000,REDIS_TIMEOUT=800,MAX_KEYS=50,WARM_CACHE_MS=30000;
...
export async function warmAiKeyPool(prefix='GEMINI'){
  ...
  const now=Date.now();
  const cached=warmCache.get(prefix);
  if(cached&&cached.fingerprint===fingerprint&&cached.expiresAt>now)return true;
  const desired=JSON.stringify({version:1,fingerprint,keys:pool.map(k=>({id:k.id,fingerprint:k.fingerprint}))});
  const current=await redis(['GET',metaKey(prefix)],'',1000);
  if(current===null){
    const ok=await redis(['SET',metaKey(prefix),desired],'',1000);
...
export async function aiPoolSnapshot(prefix='GEMINI'){const warmed=await warmAiKeyPool(prefix);const pool=getAiKeyPool(prefix),now=Date.now(),remote=await load(prefix,pool);return pool.map((k,i)=>{const s=st(k.id),r=remote?.[i];if(r)for(const f of ['failures','successes','uses','openedUntil','probeUntil','lastFailure','lastUsed'])if(r[f]!==undefined)s[f]=Number(r[f])||0;return{id:k.id,fingerprint:k.fingerprint,failures:s.failures,successes:s.successes,uses:s.uses,circuit:s.openedUntil>now?'open':(s.probeUntil>now?'half-open':'closed'),retryAt:s.openedUntil||null,lastFailure:s.lastFailure||null,lastUsed:s.lastUsed||null,warmed};});}
