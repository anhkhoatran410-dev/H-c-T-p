...
(function(){
  ...
  function install(){
    ...
    window.toggleSupportPicker=function(id){
      ...
      document.querySelectorAll('.support-picker').forEach(function(x){if(x!==t)x.classList.add('hidden')});
    ...
    document.addEventListener('click',function(e){
      ...
      if(b){
        var p=b.closest('.support-picker');
        if(p&&p.id==='supportStickerPicker'){e.preventDefault();e.stopImmediatePropagation();window.sendSticker(b.textContent||'');return}
        if(p&&p.id==='supportEmojiPicker'){e.preventDefault();e.stopImmediatePropagation();window.insertSupportEmoji(b.textContent||'');return}
      }
    ...
    if(root)new MutationObserver(function(){hardenText();dedupeBotDom();closePickers()}).observe(root,{childList:true,subtree:true});
    dedupeBotDom();
    setTimeout(dedupeBotDom,50);setTimeout(dedupeBotDom,250);setTimeout(dedupeBotDom,1000);
  }
