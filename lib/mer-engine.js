...
async function ask(model,prompt,image,timeout,maxOutputTokens){
  ...
  try{
    ...
    if(!r.ok){const e=new Error(String(d?.error?.message||('Gemini HTTP '+r.status)));e.status=r.status;e.code=d?.error?.code||undefined;e.providerMessage=d?.error?.message;if(!usingNetworkGuard)await reportAiFailure('GEMINI',apiEntry.id,e);throw e;}
    const finishReason=String(d?.candidates?.[0]?.finishReason||'');
    const truncated=finishReason==='MAX_TOKENS';
    const answer=extract(d);
    if(!answer){const e=new Error(truncated?'Gemini hết token trước khi viết lời giải (MAX_TOKENS).':'Gemini returned empty output.');e.code=truncated?'AI_TRUNCATED_EMPTY':'AI_EMPTY';if(!usingNetworkGuard)await reportAiFailure('GEMINI',apiEntry.id,e);throw e;}
    if(!usingNetworkGuard)await reportAiSuccess('GEMINI',apiEntry.id);
    return {answer,model,truncated,finishReason,keyId:actualKeyId};
  }catch(e){
    if(!usingNetworkGuard&&apiEntry&&e?.status==null&&e?.code)await reportAiFailure('GEMINI',apiEntry.id,e);
    throw e;
  }
}
