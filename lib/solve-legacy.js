...
async function run() {
  ...
  const aiCacheKey=exactCacheEligible?cacheKey({message,subject,tier:tier.tier}):null;
  if(aiCacheKey){
  ...
  let cas=''; const needsCas = math && !image && !olympiad && /(?:x\s*[=<>]|\bsolve\b|\bgiải\b|phương trình|hệ phương trình|tích phân|đạo hàm|log|ln|sin|cos|tan|căn|sqrt|\^)/i.test(message); if(needsCas){const casStarted=Date.now();const w=await wolfram(message);if(w.available)cas=w.result;stages.casMs=Date.now()-casStarted;}
  ...
  if(!merUsed&&olympiad){const auditStarted=Date.now();audit=await verify({message,subject,image,cas,candidate});stages.auditMs=Date.now()-auditStarted;}
  ...
  if(aiCacheKey&&answer)await setCached(aiCacheKey,responsePayload);
