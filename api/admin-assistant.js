...
async function projectContext(clientContext){
  const codeFiles=[
    "index.html","app.js","app-loader.js","support-fix.js","public-enhancements.js","public-final-fix.js","public-hardening.js","public-last-fix.js",
    "admin/index.html","admin/app.js","admin-final-fix.js","admin-last-fix.js","admin-final-fix.css"
  ];
  const [exams,participants,attempts,threads,accounts,rules,...code] = await Promise.all([
    supabaseFetch("exams?select=id,title,subject,difficulty,duration,question_count,status,created_at&order=created_at.desc&limit=50"),
    supabaseFetch("participants?select=id,name,code,email,created_at&order=created_at.desc&limit=200"),
    supabaseFetch("user_attempts?select=id,device_id,exam_id,exam_title,student_name,student_code,score,correct,total,duration_seconds,auto_submitted,created_at&order=created_at.desc&limit=200"),
    supabaseFetch("support_threads?select=id,device_id,student_name,account_id,status,last_message,unread_admin,unread_user,bot_waiting_admin,updated_at&order=updated_at.desc&limit=100"),
  ...
  const sourceFiles={};
  codeFiles.forEach((name,i)=>{if(code[i])sourceFiles[name]=code[i]});
  ...
  return {
    ...
    ui:{pages:["home","subject","exam","result","history","review","support","admin"],adminTabs:["dashboard","support","participants","history","tests","accounts","bot","assistant"],themes:["light","dark"]},
    data:{exams:exams||[],participants:participants||[],recentAttempts:attempts||[],supportThreads:threads||[],supportAccounts:accounts||[],botRules:rules||[]},
    live:clientContext||{}
  };
}
...
CONTEXT JSON:\n${JSON.stringify(context).slice(0,120000)}`;
  ...
  try{
    const response=await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash-lite:generateContent",{
      ...
      body:JSON.stringify({contents:[{role:"user",parts:[{text:prompt}]}],generationConfig:{temperature:0.25,maxOutputTokens:1800}})
    });
    ...
    return res.status(200).json({answer,contextSummary:{exams:(context.data.exams||[]).length,participants:(context.data.participants||[]).length,attempts:(context.data.recentAttempts||[]).length,threads:(context.data.supportThreads||[]).length}});
  }catch(e){
