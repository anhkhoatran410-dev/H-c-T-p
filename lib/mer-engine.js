...
async function ask(model,prompt,image,timeout,maxOutputTokens){
  ...
  try{
    ...
    if(!r.ok){
      const e=new Error(String(d?.error?.message||('Gemini HTTP '+r.status)));
      e.status=r.status;
      e.code=d?.error?.code||undefined;
      e.providerMessage=d?.error?.message;
      if(!usingNetworkGuard){
        reportAiFailureAsync('GEMINI',apiEntry.id,e);
        reportFailureDone=true;
      }
      throw e;
    }
    ...
    if(!answer){
      const e=new Error(truncated?'Gemini hết token trước khi viết lời giải (MAX_TOKENS).':'Gemini returned empty output.');
      e.code=truncated?'AI_TRUNCATED_EMPTY':'AI_EMPTY';
      if(!usingNetworkGuard){
        reportAiFailureAsync('GEMINI',apiEntry.id,e);
        reportFailureDone=true;
      }
    ...
    if(!usingNetworkGuard&&apiEntry&&e?.status==null&&e?.code&&!reportFailureDone){
      reportAiFailureAsync('GEMINI',apiEntry.id,e);
      reportFailureDone=true;
    }
    throw e;
