...
async function solveCompoundTasks({tasks,subject,history,image,forcedDeep}) {
  ...
  deep.forEach(item=>jobs.push((async()=>{
    try{
      const candidate=await solveWithFallback({message:item.task,subject,history:[],image,deep:true,tier:'deep',codeExecutionTools:subjectMode(subject,item.task)==='math'});
      return {kind:'deep',candidate,index:item.index,task:item.task};
  ...
  for(const job of done){
    if(job.kind==='deep'){
      if(job.candidate)results[job.index]={answer:job.candidate.answer,model:job.candidate.model,providerLatencyMs:job.candidate.providerLatencyMs,deep:true};
