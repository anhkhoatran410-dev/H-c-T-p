...
async function ask(model,prompt,image,timeout,maxOutputTokens){
  ...
  if(!answer){const e=new Error(truncated?'Gemini hết token trước khi viết lời giải (MAX_TOKENS).':'Gemini returned empty output.');e.code=truncated?'AI_TRUNCATED_EMPTY':'AI_EMPTY';throw e;}
  return {answer,model,truncated,finishReason};
}
...
function escapeRe(s){return String(s).replace(/[.*+?^${}()|[\]\\]/g,'\\function escapeRe(s){return String(s).replace(/[.*+?^${}()|[\]\\]/g,'\\function escapeRe(s){return String(s).replace(/[.*+?^$ { }()|[\]\\]/g,'\\$&')}')}')}
