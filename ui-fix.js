...
(function(){
  ...
  function sync(){
    ...
    if(!next)return;
    if(next!==list){
      if(mo)mo.disconnect();if(ro)ro.disconnect();list=next;stick=true;
  ...
  window.reviewPage=function(){
    ...
    var item=x.items[x.cursor],q=item.question,correct='';
    if(q.type==='mcq')correct=String.fromCharCode(65+Number(q.a||0))+'. '+String(q.opts?.[Number(q.a||0)]||'');
    else if(q.type==='short')correct=String(q.answer||'');
    else correct=(q.answers||[]).map(function(v,j){return String.fromCharCode(97+j)+'. '+(v?'Đúng':'Sai')}).join(' · ');
    var content='';
