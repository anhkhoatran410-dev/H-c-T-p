...
async function repair({ message, subject, image, cas, candidate, audit, attempt, codeExecutionTools, visual }) { if (!candidate) return null; return solveWithFallback({ message: `REPAIR ENGINE — GIẢI LẠI TOÀN BỘ, không vá một dòng. Lời giải ứng viên đã FAIL. Dùng Code Execution khi Router đã cho phép hoặc khi phần lỗi cần xác nhận bằng tính toán; bằng chứng cuối phải là lập luận toán học. Sửa toàn bộ lỗi trong kiểm định và tự kiểm tra lại. Vòng sửa ${attempt}.\n\nĐỀ GỐC:\n${message}\n\nLỜI GIẢI CŨ:\n${candidate.answer}\n\nKIỂM ĐỊNH:\n${JSON.stringify(audit)}`, subject, history: [], image, cas, deep: true, codeExecutionTools: codeExecutionTools === true, visual, timeoutOverride: REPAIR_TIMEOUT_MS, maxOutputTokensOverride: 9000 }); }
...
async function run() {
  ...
  const visual = visualIntent(subject, message, image);
  ...
  try {
    ...
    candidate = await openaiFallback({ message, subject, image, cas, visual });
  ...
  if (autoDeep && (math || olympiad)) {
    ...
    for (let i = 1; i <= MAX_REPAIR_ROUNDS && audit?.verdict === 'FAIL' && (Date.now() - started) < 40000; i++) {
      const repaired = await repair({ message, subject, image, cas, candidate, audit, attempt: i, codeExecutionTools, visual });
      if (!repaired) break;
      candidate = repaired;
      repairCount++;
