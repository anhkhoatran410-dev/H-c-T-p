/* STUDY Admin — single authoritative login guard. */
(function () {
  'use strict';
  ...
  function repair() {
    ...
    button.style.pointerEvents = submitting ? 'none' : 'auto';
    return true;
  }
      ...
      setTimeout(() => {
        try {
          if (typeof window.bootAdmin === 'function') {
            Promise.resolve(window.bootAdmin()).catch(err => console.error('[STUDY Admin] boot error:', err));
          }
        } catch (err) { console.error('[STUDY Admin] boot error:', err); }
      }, 0);
    } catch (error) {
      console.error('[STUDY Admin] login error:', error);
      message(error?.name === 'AbortError'
        ? '❌ Máy chủ đăng nhập không phản hồi sau 8 giây. Kiểm tra API /api/admin-login.'
