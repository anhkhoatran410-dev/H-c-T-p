  ...
  try{
    ...
    const r=await fetch(`${AUDIT_REDIS_URL}/pipeline`,{
      ...
      body:JSON.stringify([[
        ...
        AUDIT_RECENT_KEY,
        AUDIT_DROPPED_KEY,
        String(AUDIT_QUEUE_MAX),
        payload,
        String(AUDIT_COUNTER_TTL_SECONDS),
        String(AUDIT_RECENT_MAX - 1),
      ]]),
...
export async function persistAudit(row){
  // Audit remains best-effort. Queue admission is atomic and bounded so audit load
  // cannot consume the unbounded memory of the shared security state.
