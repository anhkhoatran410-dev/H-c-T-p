...
const DEEP_MODELS = ['gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3.8-flash'];
...
const DEEP_SOLVER_TIMEOUT_MS = 20000;
...
function splitCompoundTasks(message) {
  const raw=String(message||'').split(/\r?\n/).map(s=>s.trim()).filter(Boolean);
  const cleaned=raw.map(s=>s.replace(/^(?:[-•*]|\d+[.)]|[a-z][.)])\s*/i,'').trim()).filter(Boolean);
  return cleaned.length>=2 && cleaned.length<=8 ? cleaned : null;
}
function isStandaloneGraphTask(task) {
  const s=String(task||'').trim();
  return /^\s*(?:tôi\s+muốn\s+)?(?:hãy\s+)?(?:hình\s+dung|vẽ|biểu\s+diễn|tạo|cho\s+mình\s+)?(?:đồ\s+thị|graph|plot)\b[\s:,-]*(?:y\s*=\s*)?(?:sin|cos|tan)\s*\(?(?:x)\)?\s*[.!?]*\s*$/i.test(s);
}
function looksDeepTask(subject,task) {
  return isOlympiadMath(subject,task) || isDeepTheoryCandidate(subject,task,false) || /\b(?:VMO|MATH|IMO|Olympiad)\b/i.test(String(task||''));
}
async function solveCompoundTasks({tasks,subject,history,image,forcedDeep}) {
  const started=Date.now();
  const results=new Array(tasks.length).fill(null);
  const pending=[];
  tasks.forEach((task,index)=>{
    const localGraph=localGraphReply(task);
    if(localGraph&&isStandaloneGraphTask(task)&&!image) results[index]={answer:localGraph,local:true,model:'local-graph'};
    else pending.push({task,index,deep:forcedDeep||looksDeepTask(subject,task)});
  });

  const normal=pending.filter(x=>!x.deep);
  const deep=pending.filter(x=>x.deep);
  const jobs=[];
  if(normal.length){
    const bundle=normal.map((x,i)=>`CÂU ${i+1}:\n${x.task}`).join('\n\n');
    jobs.push((async()=>{
      try{
        const candidate=await solveWithFallback({message:'Hãy trả lời TẤT CẢ các câu dưới đây. Giữ đúng thứ tự, đánh số CÂU 1, CÂU 2... Không bỏ sót câu nào. Trả lời ngắn gọn nhưng đủ bước.\n\n'+bundle,subject,history,image,deep:false,tier:'fast',codeExecutionTools:false});
        return {kind:'normal',candidate,items:normal};
      }catch(_){return {kind:'normal',candidate:null,items:normal};}
    })());
  }
  deep.forEach(item=>jobs.push((async()=>{
    try{
      let candidate=await solveWithFallback({message:item.task,subject,history:[],image,deep:true,tier:'deep',codeExecutionTools:subjectMode(subject,item.task)==='math'});
      const isTheory=isTheorySubject(subject,item.task);
      let audit=isTheory
        ? await verifyTheory({message:item.task,subject,image,candidate})
        : await verify({message:item.task,subject,image,cas:'',candidate});
      if(audit?.verdict==='FAIL'){
        const repaired=await repair({message:item.task,subject,image,cas:'',candidate,audit,attempt:1,codeExecutionTools:subjectMode(subject,item.task)==='math',visual:visualIntent(subject,item.task,image),theory:isTheory});
        if(repaired){candidate=repaired; const reAudit=isTheory ? await verifyTheory({message:item.task,subject,image,candidate}) : await verify({message:item.task,subject,image,cas:'',candidate}); audit=reAudit||audit;}
      }
      return {kind:'deep',candidate,index:item.index,task:item.task,audit};
    }catch(_){return {kind:'deep',candidate:null,index:item.index,task:item.task};}
  })()));
  ...
  for(const job of done){
    if(job.kind==='deep'){
      if(job.candidate)results[job.index]={answer:job.candidate.answer,model:job.candidate.model,providerLatencyMs:job.candidate.providerLatencyMs,deep:true,auditVerdict:job.audit?.verdict||null};
      else {results[job.index]={answer:'Không thể xử lý câu này lúc này.',error:true};providerFailure=true;}
    }else{
      if(job.candidate){
        const text=String(job.candidate.answer||'').trim();
        const chunks=text.split(/(?=CÂU\s*\d+\s*:)/ig).map(s=>s.trim()).filter(Boolean);
        job.items.forEach((item,idx)=>{
          const expected=new RegExp('^CÂU\\s*'+(idx+1)+'\\s*:', 'i');
          const found=chunks.find(x=>expected.test(x));
          results[item.index]={answer:found?found.replace(/^CÂU\s*\d+\s*:\s*/i,'').trim():text,model:job.candidate.model,providerLatencyMs:job.candidate.providerLatencyMs,deep:false};
        });
      }else{
        job.items.forEach(item=>results[item.index]={answer:'Không thể xử lý câu này lúc này.',error:true});
        providerFailure=true;
      }
    }
  }
  return {answer:results.map((r,i)=>`**Câu ${i+1}**\n${r?.answer||'Không có kết quả.'}`).join('\n\n'),results,totalLatencyMs:Date.now()-started,providerFailure,compound:true};
}
function localFastReply(message) {
  const s=String(message||'').trim().toLowerCase().replace(/[!?.,]+$/g,'');
  if(/^(hi|hello|hey|chào|chao|xin chào|xin chao|alo|hí|hê lô|helo)$/.test(s)) return 'Chào bạn 👋 Mình đang sẵn sàng hỗ trợ học tập. Bạn gửi bài hoặc câu hỏi mình sẽ xử lý ngay.';
  if(/^(cảm ơn|cam on|thanks|thank you)$/.test(s)) return 'Không có gì 👌 Gửi bài tiếp theo khi cần nhé.';
  return null;
}
async function run() {
  ...
  const localReply=!body.imageDataUrl?localFastReply(message):null;
  if(localReply)return json(res,200,{answer:localReply,model:'local-fast',processingTier:'local',deepMode:false,olympiadMode:false,theoryMode:false,totalLatencyMs:Date.now()-started,providerLatencyMs:0}); const subject=String(body.subject||'');
  ...
  const compoundTasks=!image?splitCompoundTasks(message):null;
  if(compoundTasks && compoundTasks.length>=2){
    const batch=await solveCompoundTasks({tasks:compoundTasks,subject,history,image,forcedDeep});
    return json(res,200,{answer:batch.answer,model:'batch',processingTier:'batch',deepMode:Boolean(forcedDeep||batch.results.some(x=>x?.deep)),olympiadMode:batch.results.some(x=>looksDeepTask(subject,x?.answer||'')),theoryMode:isTheorySubject(subject,message),compound:true,batchResults:batch.results.map(x=>({model:x?.model||null,deep:Boolean(x?.deep),error:Boolean(x?.error)})),totalLatencyMs:batch.totalLatencyMs,providerLatencyMs:null});
  }
