...
(function(){
  ...
  function validate(qs,count,types){
    ...
    if(isFlashOnly){
      if(!qs.length)throw new Error('AI không tạo được flashcard nào.');
      if(qs.length<count)throw new Error('AI chỉ trích xuất được '+qs.length+'/'+count+' flashcard từ tài liệu. Chưa lưu để tránh thiếu thẻ.');
      qs.splice(count);
    }else if(qs.length!==count)throw new Error('AI tạo '+qs.length+'/'+count+' nội dung. Chưa lưu để tránh đề thiếu câu.');
    var bad=[];
  ...
  function prepareImage(file){
    return new Promise(async function(resolve,reject){
      try{
        ...
        img.onload=function(){
          var max=1800,scale=Math.min(1,max/Math.max(img.naturalWidth||img.width,img.naturalHeight||img.height)),w=Math.max(1,Math.round((img.naturalWidth||img.width)*scale)),h=Math.max(1,Math.round((img.naturalHeight||img.height)*scale));var canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;var ctx=canvas.getContext('2d',{alpha:false});
          if(!ctx){resolve({data:raw,mime:file.type||'image/jpeg'});return}ctx.fillStyle='#fff';ctx.fillRect(0,0,w,h);ctx.drawImage(img,0,0,w,h);resolve({data:canvas.toDataURL('image/jpeg',.74),mime:'image/jpeg'});
  ...
  async function renderPdfPages(file){
    ...
    var pdf=await window.pdfjsLib.getDocument({data:await file.arrayBuffer()}).promise,pages=[];
    for(var i=1;i<=pdf.numPages;i++){
      var page=await pdf.getPage(i),base=page.getViewport({scale:1}),max=1800,scale=Math.min(1.55,max/base.width),vp=page.getViewport({scale:scale}),canvas=document.createElement('canvas');canvas.width=Math.ceil(vp.width);canvas.height=Math.ceil(vp.height);var ctx=canvas.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,canvas.width,canvas.height);await page.render({canvasContext:ctx,viewport:vp}).promise;pages.push({data:canvas.toDataURL('image/jpeg',.74),mime:'image/jpeg',page:i});
    }
    return pages;
  }
  async function getInputPayload(files,msg){
    var list=Array.from(files||[]);if(!list.length)throw new Error('Hãy chọn ít nhất một tài liệu hoặc ảnh.');var texts=[],images=[];
    for(var fi=0;fi<list.length;fi++){
      ...
      if(isPdf){if(msg)msg.textContent='⏳ Đang quét PDF '+(fi+1)+'/'+list.length+' thành ảnh để AI Vision đọc bảng...';var pages=await renderPdfPages(file);pages.forEach(function(p){images.push(p)});continue}
      if(msg)msg.textContent='⏳ Đang đọc tài liệu '+(fi+1)+'/'+list.length+'...';var text='';try{text=await extractDoc(file)}catch(e){text=''}if(text)texts.push('--- '+file.name+' ---\n'+String(text));
    }
  ...
  async function createByAI(){
    ...
    if(!files||!files.length){msg.textContent='⚠️ Hãy chọn tài liệu hoặc ảnh.';return}if(!title){msg.textContent='⚠️ Hãy đặt tên bài.';return}if(!types.length){msg.textContent='⚠️ Chọn ít nhất một dạng nội dung.';return}if(!Number.isInteger(count)||count<1||count>100){msg.textContent='⚠️ Số lượng phải từ 1 đến 100.';return}
    var flashOnly=types.length===1&&types[0]==='flashcard';
    try{
      var selected=await getInputPayload(files,msg);if(!selected.documentText&&!selected.fileData.length)throw new Error('Không đọc được nội dung. Hãy dùng tài liệu có chữ hoặc ảnh rõ nét.');var questions=[];
      if(flashOnly){
        ...
        var response=await fetch('/api/generate-exam',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fileName:Array.from(files).map(function(f){return f.name}).join(', '),mimeType:'image/jpeg',mimeTypes:selected.fileMimeTypes,documentText:selected.documentText,fileData:selected.fileData,subject:subject,difficulty:difficulty,questionCount:count,types:types})});var data=await response.json().catch(function(){return{}});if(!response.ok)throw new Error(data.error||'Không tạo được đề');questions=normalizeQuestions(data.questions);validate(questions,count,types);
      }
      await loadSupabase();var insert=await db.from('exams').insert({title:title,subject:subject,difficulty:difficulty,duration:flashOnly?0:duration,question_count:questions.length,questions:questions,status:'active'}).select('*').single();if(insert.error)throw insert.error;if(!insert.data||!Array.isArray(insert.data.questions)||insert.data.questions.length!==questions.length)throw new Error('Máy chủ lưu dữ liệu không đủ. Chưa báo thành công.');
      msg.textContent=flashOnly?'✅ Đã tạo '+questions.length+' flashcard. Mỗi từ/cụm từ là một thẻ 2 mặt.':'✅ Đã tạo '+questions.length+' nội dung và lưu thành công.';document.getElementById('title').value='';document.getElementById('file').value='';if(typeof renderTests==='function')await renderTests();if(typeof loadDashboard==='function')loadDashboard();
    }catch(err){console.error(err);msg.textContent='❌ '+(err&&err.message||err)}
  }
  function boot(){var btn=document.getElementById('createBtn');if(!btn||btn.dataset.hardened==='1')return;btn.dataset.hardened='1';btn.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();createByAI()},true)}
...
(function(){
  var scheduled=false;
  function fitSupportMessenger(){scheduled=false;var support=document.getElementById('support'),messenger=support&&support.querySelector('.messenger');if(!messenger)return;var rect=messenger.getBoundingClientRect(),available=Math.floor(window.innerHeight-rect.top-12),min=window.innerWidth<=650?360:(window.innerWidth<=900?390:0);if(available<min)available=min;messenger.style.setProperty('height',Math.max(available,0)+'px','important');messenger.style.setProperty('max-height',Math.max(available,0)+'px','important');messenger.style.setProperty('min-height','0px','important');messenger.style.setProperty('overflow','hidden','important');var conversation=messenger.querySelector('.conversation');if(conversation){conversation.style.setProperty('height','100%','important');conversation.style.setProperty('min-height','0','important');conversation.style.setProperty('overflow','hidden','important')}var messages=document.getElementById('supportMessages');if(messages){messages.style.setProperty('min-height','0','important');messages.style.setProperty('overflow-y','auto','important');messages.style.setProperty('overflow-x','hidden','important')}var composer=document.getElementById('replyForm');if(composer){composer.style.setProperty('display','flex','important');composer.style.setProperty('visibility','visible','important');composer.style.setProperty('opacity','1','important');composer.style.setProperty('position','relative','important');composer.style.setProperty('bottom','auto','important');composer.style.setProperty('z-index','100000','important')}}
