import { acquireAiKey, getAiKeyPool, reportAiFailureAsync, reportAiSuccessAsync } from './api/_ai-resilience.js';
...
function models(){
  ...
  return [...new Set([configured,...extra,...DEFAULT_MODELS].filter(Boolean))];
}
...
async function ask(model,prompt,image,timeout,maxOutputTokens){
  ...
  try{
    const requestInit={method:'POST',headers,body:JSON.stringify({contents:[{role:'user',parts}],generationConfig:{maxOutputTokens,thinkingConfig:{thinkingLevel:'high'}}}),signal:AbortSignal.timeout(timeout)};
    if(usingNetworkGuard)requestInit.timeoutMs=timeout;
    const r=await fetch('https://generativelanguage.googleapis.com/v1beta/models/'+model+':generateContent',requestInit);
    const actualKeyId=r.headers.get('X-Study-Th-Key-Id')||apiEntry?.id||null;
    const raw=await r.text();let d={};try{d=raw?JSON.parse(raw):{}}catch{}
    if(!r.ok){const e=new Error(String(d?.error?.message||('Gemini HTTP '+r.status)));e.status=r.status;e.code=d?.error?.code||undefined;e.providerMessage=d?.error?.message;if(!usingNetworkGuard)reportAiFailureAsync('GEMINI',apiEntry.id,e);throw e;}
    const finishReason=String(d?.candidates?.[0]?.finishReason||'');
    const truncated=finishReason==='MAX_TOKENS';
    const answer=extract(d);
    if(!answer){const e=new Error(truncated?'Gemini hết token trước khi viết lời giải (MAX_TOKENS).':'Gemini returned empty output.');e.code=truncated?'AI_TRUNCATED_EMPTY':'AI_EMPTY';if(!usingNetworkGuard)reportAiFailureAsync('GEMINI',apiEntry.id,e);throw e;}
    if(!usingNetworkGuard)reportAiSuccessAsync('GEMINI',apiEntry.id);
    return {answer,model,truncated,finishReason,keyId:actualKeyId};
  }catch(e){
    if(!usingNetworkGuard&&apiEntry&&e?.status==null&&e?.code)reportAiFailureAsync('GEMINI',apiEntry.id,e);
    throw e;
  }
}
...
async function runExperts(prompt,image,timeout,tokens,onStage=null){
  ...
  try{if(typeof onStage==='function')await onStage('experts_started',{models:models().slice(0,2),timeoutMs:timeout})}catch{}
  const results=await Promise.allSettled(models().slice(0,2).map(model=>ask(model,prompt,image,timeout,tokens)));
...
async function runReview(prompt,image,timeout,tokens,askFn=ask){
  const started=Date.now();let last=null;
  for(const model of models().slice(0,2)){
    const remaining=timeout-(Date.now()-started);
    // Intentional boundary behavior: a non-positive review budget means no model attempt.
    // Current merSolve callers provide positive profile budgets; future callers must preserve that contract.
    if(remaining<=0)break;
    const attemptTimeout=Math.min(5000,remaining);
    try{
      const r=await askFn(model,prompt,image,attemptTimeout,tokens);
...
export const __test={ask,runReview,casSupports,conclusionSignature,agreementScore,proofLike};
