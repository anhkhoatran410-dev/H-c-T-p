...
(function(){
  ...
  async function startAdmin(){if(window.__studyAdminStarted)return;window.__studyAdminStarted=true;try{await loadScript(CRITICAL_SCRIPT);await loadScript(APP_SCRIPT);setAuthenticatedView();bindNavigation();if(typeof window.bootAdmin==='function'){try{await window.bootAdmin()}catch(e){console.warn('[STUDY boot]',e)}}}catch(e){window.__studyAdminStarted=false;console.error('[STUDY bootstrap]',e);setAuthenticatedView();bindNavigation();const t=$('toast');if(t){t.textContent='⚠️ Admin mở nhưng một số mô-đun phụ chưa tải.';t.classList.add('show')}}}
