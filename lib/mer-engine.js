...
async function ask(model,prompt,image,timeout,maxOutputTokens){
  ...
  try{
    ...
    if(!r.ok){const e=new Error(String(d?.error?.message||('Gemini HTTP '+r.status)));e.status=r.status;e.providerMessage=d?.error?.message;if(!usingNetworkGuard)await reportAiFailure('GEMINI',apiEntry.id,e);throw e;}
