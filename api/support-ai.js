...
export default async function handler(req,res){
  ...
  const first=acquireAiKey('GEMINI');
  if(!first)return res.status(500).json({error:'Chưa cấu hình GEMINI_API_KEY hoặc pool API key.'});
  const poolIds=[];
  const noteFailure=(keyEntry,error)=>{ if(keyEntry){ poolIds.push(keyEntry.id); reportAiFailure('GEMINI',keyEntry.id,error); } };
  const noteSuccess=(keyEntry)=>{ if(keyEntry)reportAiSuccess('GEMINI',keyEntry.id); };

  const badIndex=[...first.key].findIndex(ch=>ch.charCodeAt(0)>127);
  if(badIndex>=0){
    noteFailure(first,{status:401});
    return res.status(500).json({error:'GEMINI_API_KEY trên Vercel chứa ký tự không hợp lệ.'});
  }
  ...
  try{
    // Primary path: Gemini Interactions API. Rotate to another configured key on failure.
    for(let keyAttempt=0;keyAttempt<Math.max(1,20);keyAttempt++){
      const keyEntry=keyAttempt===0?first:acquireAiKey('GEMINI',Array.from(attempted));
      if(!keyEntry)break;
      attempted.add(keyEntry.id);
      ...
      try{
        ...
        if(interaction.ok){
          ...
          if(answer){noteSuccess(keyEntry);return res.status(200).json({answer,model:data?.model||'gemini-3.8-flash',api:'interactions'});}
          last='Interactions API trả về rỗng.';
          noteFailure(keyEntry,{status:502});
        }else{
          last=providerMessage(data,interaction.status);
          noteFailure(keyEntry,{status:interaction.status});
        }
      }catch(e){
        last=e?.message||'Gemini Interactions API lỗi.';
        noteFailure(keyEntry,{status:e?.status||0});
      }
    }

    // Compatibility fallback. Each model attempt may use the next healthy key.
    for(const model of MODELS){
      const keyEntry=acquireAiKey('GEMINI',Array.from(attempted));
      if(!keyEntry)break;
      attempted.add(keyEntry.id);
      const key=cleanKey(keyEntry.key);
      try{
        ...
        if(r.ok){
          ...
          if(answer){noteSuccess(keyEntry);return res.status(200).json({answer,model});}
          last=`${model}: AI trả về rỗng.`;
          noteFailure(keyEntry,{status:502});
        }else{
          last=providerMessage(data2,r.status);
          noteFailure(keyEntry,{status:r.status});
        }
      }catch(e){
