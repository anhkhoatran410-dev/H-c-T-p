      ...
      const submitQuestion=async function(e){
        ...
        try{
          ...
          if(backgroundDeep){
            ...
            // Fast path: use normal JSON instead of SSE streaming.
            // The backend already has its own fallback; avoiding SSE here removes
            // mobile/webview stream buffering and BodyStreamBuffer abort problems.
            const maxAttempts=3;
            let lastErr=null;
            for(let attempt=0;attempt<maxAttempts;attempt++){
              try{
                ...
                try{
                  ...
                  d=contentType.includes('application/json')?await r.json().catch(()=>({})) : {error:await r.text()};
                  if(r.ok&&d?.answer)break;
                  lastErr=Object.assign(new Error(String(d.error||('Solver HTTP '+r.status))),{
                    status:Number(d.status)||r.status,
                    providerStatus:d.providerStatus,
                    code:d.code,
                    retryable:d.retryable,
                    requestId:d.requestId||r.headers.get('x-request-id')||''
                  });
                ...
                if(fetchErr?.name==='AbortError'){
                  lastErr=Object.assign(new Error('AI phản hồi quá lâu. Hệ thống đã thử lại nhưng chưa nhận được kết quả.'),{status:504,retryable:true});
                }else{
                  lastErr=fetchErr;
                }
