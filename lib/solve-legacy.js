...
async function solveWithFallback(args) {
  const models = args.deep ? GEMINI_MODELS : GEMINI_MODELS.slice(0, 2);
  const attemptsPerModel = args.deep ? 2 : 1;
  let last = null;
  const started = Date.now();
  const budget = args.deep ? 52000 : 48000;
...
async function run() {
  ...
  let cas=''; if(math&&!image){const w=await wolfram(message);if(w.available)cas=w.result;}
