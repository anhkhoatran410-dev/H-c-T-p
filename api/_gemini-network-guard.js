...
if (!globalThis.__STUDY_TH_GEMINI_GUARD__) {
  ...
  globalThis.fetch = async (input, init = {}) => {
    ...
    let parsed; try { parsed = new URL(url); } catch (_) { parsed = null; }
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
      ...
      try {
        ...
        if (response.ok) { reportAiSuccess('GEMINI', entry.id); return response; }
        const status = Number(response.status || 0);
        if (!shouldRotate(status)) return response;
        reportAiFailure('GEMINI', entry.id, { status });
        lastError = { status };
      } catch (error) {
        reportAiFailure('GEMINI', entry.id, { status: Number(error?.status || 0), code: error?.code });
        lastError = error;
        if (attempt + 1 >= MAX_ATTEMPTS) throw error;
      }
    }
    if (lastError?.status) {
      return new Response(JSON.stringify({ error: { status: lastError.status, message: 'All Gemini keys are temporarily unavailable.' } }), { status: lastError.status, headers: { 'content-type': 'application/json; charset=utf-8' } });
    }
