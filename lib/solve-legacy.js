    ...
    for (let attempt = 0; attempt < maxAttemptsPerModel; attempt++) {
      try {
        ...
        return await gemini({
          ...
          model,
          deep: args.deep === true,
          maxOutputTokens: args.deep === true ? MAX_OUTPUT_TOKENS : 5000,
          timeout: args.deep === true ? 50000 : 18000,
...
async function run() {
  ...
  const explicitDeep = body.deep === true;

  // Auto-route: ordinary questions stay on the fast path.
  // Deep is reserved for explicit "Kiểm tra kỹ" or signals of VMO/Olympiad/complex math.
  const complexMath = math && (
    message.length > 420 ||
    /\n/.test(message) ||
    /(?:chứng minh|prove|bất đẳng thức|inequality|số học|number theory|tổ hợp|combinatorics|hình học|geometry|tham số|parameter|biện luận|đếm|count|modulo|đồng dư|recursive|quy nạp)/i.test(message)
  );
  const deep = explicitDeep || olympiad || complexMath;

  let route = null;
  let cas = '';

  // Only pay the router/CAS cost on the deep path. They run concurrently.
  if (deep) {
    const routePromise = analyzeRoute({ message, subject, image });
    const casPromise = math && !image ? wolfram(message) : Promise.resolve({ available: false });
    const [r, w] = await Promise.all([routePromise, casPromise]);
    route = r;
  ...
  if (!route) {
    route = {
      needs_code: false,
      reason: deep ? 'Router unavailable; safe deep no-code fallback.' : 'Fast path: router skipped.',
      problem_type: math ? 'other' : 'general',
      difficulty: olympiad ? 'HARD' : (deep ? 'MEDIUM' : 'EASY'),
      model: null,
      used: false
    };
  }

  const codeExecutionTools = deep && route.needs_code === true;
  let candidate = null;
  let source = 'gemini';
  let audit = null;
  let repairCount = 0;
  ...
  // Verification is valuable for VMO/deep work, not for every normal question.
  const shouldVerify = deep && (olympiad || explicitDeep || String(route.difficulty || '').toUpperCase() === 'HARD' || String(route.difficulty || '').toUpperCase() === 'CHALLENGE');
  if (shouldVerify) {
    audit = await verify({ message, subject, image, cas, candidate });
    if (audit?.verdict === 'FAIL') {
      const repaired = await repair({
        message, subject, image, cas, candidate, audit,
        attempt: 1, codeExecutionTools
      });
      if (repaired) {
        candidate = repaired;
        repairCount = 1;
  ...
  return json(res, 200, {
    answer,
    route: deep ? (shouldVerify ? 'deep-verified' : 'deep') : 'fast',
