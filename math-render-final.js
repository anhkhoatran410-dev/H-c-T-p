...
(function(){
  if(window.__studyMathRenderFinal)return;
  window.__studyMathRenderFinal=true;
  ...
  function normalize(root){var w=document.createTreeWalker(root,NodeFilter.SHOW_TEXT),a=[],n;while(n=w.nextNode())a.push(n);a.forEach(function(x){if(skip(x))return;var t=x.nodeValue||'';if(!t.trim()||!/\\(?:frac|dfrac|sqrt|infty|leq|geq|neq|approx|equiv|times|cdot|div|pm|rightarrow|Rightarrow|leftrightarrow|int|sum|prod|partial|pi|theta|alpha|beta|gamma|delta|Delta|lambda|mu|sigma|omega|Omega|text|mathrm)|\^\{?[23n]\}?/.test(t))return;var p=pretty(t);if(p!==t){var s=document.createElement('span');s.setAttribute('data-study-math','1');s.className='study-math-text';s.textContent=p;x.parentNode.replaceChild(s,x)}})}
  function render(){var root=document.getElementById('app')||document.body;normalize(root);load().then(function(){if(window.renderMathInElement){try{window.renderMathInElement(root,{delimiters:[{left:'$$',right:'$$',display:true},{left:'\\(',right:'\\)',display:false},{left:'\\[',right:'\\]',display:true}],throwOnError:false})}catch(e){console.warn('KaTeX:',e)}}})}
  function boot(){render();var root=document.getElementById('app')||document.body;if(!root.__studyMathFinalObs){root.__studyMathFinalObs=new MutationObserver(function(){clearTimeout(root.__studyMathFinalTimer);root.__studyMathFinalTimer=setTimeout(render,40)});root.__studyMathFinalObs.observe(root,{childList:true,subtree:true)}}}
  var style=document.createElement('style');style.textContent='.study-math-text{font-family:KaTeX_Main,"Times New Roman",serif}.katex{font-size:1.07em}.katex-display{margin:.55em 0}.q .katex,.option .katex,.attempt-question .katex,.attempt-detail .katex{font-size:1.08em}';document.head.appendChild(style);
