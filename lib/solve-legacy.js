...
async function solveWithFallback(args) {
  ...
  const maxOutputTokens=args.deep ? MAX_OUTPUT_TOKENS : 6500;
  const timeout=args.deep ? 24000 : 16000;
...
async function run() { const req = globalThis.__REQ__, res = globalThis.__RES__; const body = getBody(req); if (!body.message && !body.imageDataUrl) return json(res, 400, { error: 'Thiếu đề bài hoặc ảnh.' }); const message = String(body.message || 'Giải bài trong ảnh.'); const subject = String(body.subject || ''); const history = Array.isArray(body.history) ? body.history : []; const image = String(body.imageDataUrl || ''); const deep = body.deep === true; const math = subjectMode(subject, message) === 'math'; const olympiad = isOlympiadMath(subject, message); let cas=''; let route=null; if(math&&deep){ const [w,routed]=await Promise.all([!image?wolfram(message):Promise.resolve({available:false}),analyzeRoute({message,subject,image})]); if(w?.available)cas=w.result; route=routed; } if(!route)route={needs_code:false,reason:'Router unavailable; using safe no-code fallback.',problem_type:math?'other':'general',difficulty:olympiad?'HARD':'MEDIUM',model:null,used:false}; const codeExecutionTools = route.needs_code === true; let candidate = null; let source = 'gemini'; let audit = null; let repairCount = 0; try { candidate = await solveWithFallback({ message, subject, history, image, cas, deep, codeExecutionTools }); } catch (e) { candidate = await openaiFallback({ message, subject, image, cas }); source = candidate ? 'openai' : 'none'; } if (!candidate) return json(res, 502, { error: 'Không thể tạo lời giải lúc này.' }); if (deep && (math || olympiad)) { audit = await verify({ message, subject, image, cas, candidate }); for (let i = 1; i <= MAX_REPAIR_ROUNDS && audit?.verdict === 'FAIL'; i++) { const repaired = await repair({ message, subject, image, cas, candidate, audit, attempt: i, codeExecutionTools }); if (!repaired) break; candidate = repaired; repairCount++; const reAudit = await verify({ message, subject, image, cas, candidate }); audit = reAudit || audit; if (audit?.verdict !== 'FAIL') break; } } const answer = normalizeAnswer(candidate.answer); return json(res, 200, { answer, tool: cas ? 'Wolfram + Verification' : (audit ? 'Verification' : null), model: candidate.model || null, auditVerdict: audit?.verdict || null, auditIssues: Array.isArray(audit?.issues) ? audit.issues.slice(0, 6) : [], repairEngineUsed: repairCount > 0, repairCount, source, codeExecutionUsed: Boolean(candidate.codeExecutionUsed), codeExecutionRequested: codeExecutionTools, router: route, olympiadMode: olympiad, providerLatencyMs: candidate.providerLatencyMs || null }); }
