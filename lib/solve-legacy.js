...
function reasoningTier(subject, text, deepFlag=false) {
  ...
  const olympiad=isOlympiadMath(subject,text);
  if(olympiad || /\b(?:vmo|aime|imo|olympiad|olympic|prove|chứng minh|bất đẳng thức|number theory|tổ hợp|combinatorics|hình học khó|geometry hard)\b/i.test(s)) return {tier:'deep',thinking:'high',timeout:18000,maxOutput:7000,mer:true};
  if(deepFlag || /\b(?:hard|challenge|khó|nâng cao|mức độ 3|mức độ 4)\b/i.test(s)) return {tier:'hard',thinking:'medium',timeout:14000,maxOutput:5200,mer:false};
  if(/\b(?:medium|trung bình|mức độ 2|áp dụng)\b/i.test(s)) return {tier:'standard',thinking:'low',timeout:10000,maxOutput:3600,mer:false};
...
async function run() {
  ...
  const message=String(body.message||'Giải bài trong ảnh.'),subject=String(body.subject||''),history=Array.isArray(body.history)?body.history:[],image=String(body.imageDataUrl||''),math=subjectMode(subject,message)==='math',olympiad=isOlympiadMath(subject,message),deep=body.deep===true||olympiad||/\b(?:hard|challenge|vm[op]|olympiad|olympic|prove|chứng minh|bất đẳng thức|number theory|tổ hợp|combinatorics|hình học|geometry)\b/i.test(message),tier=reasoningTier(subject,message,body.deep===true);
  let cas=''; const needsCas = math && !image && !olympiad && /(?:x\s*[=<>]|\bsolve\b|\bgiải\b|phương trình|hệ phương trình|tích phân|đạo hàm|log|ln|sin|cos|tan|căn|sqrt|\^)/i.test(message); if(needsCas){const w=await wolfram(message);if(w.available)cas=w.result;}
  ...
  try{
    if(tier.mer){
      ...
      const mer=await merSolve({message,subject,history,image,olympiad});
