...
const DEEP_SOLVER_TIMEOUT_MS = 26000;
const VERIFY_TIMEOUT_MS = 7000;
const REPAIR_TIMEOUT_MS = 10000;
const OPENAI_FALLBACK_TIMEOUT_MS = 16000;
...
async function run() {
  const req = globalThis.__REQ__, res = globalThis.__RES__;
  ...
  const forcedDeep = body.deep === true;
  const math = subjectMode(subject, message) === 'math';
  const olympiad = isOlympiadMath(subject, message);
  const deepCandidate = isDeepCandidate(subject, message, forcedDeep);
  let cas = '';
  let route = null;

  // Fast path: ordinary questions skip Router, Wolfram and Verification entirely.
  // Deep candidate: Router and CAS can work in parallel before the main solver.
  if (deepCandidate && math) {
    const [w, routed] = await Promise.all([
      !image ? wolfram(message) : Promise.resolve({ available: false }),
      analyzeRoute({ message, subject, image })
    ]);
    if (w?.available) cas = w.result;
    route = routed;
  }
  ...
  if (!route) {
    route = {
      needs_code: false,
      reason: deepCandidate ? 'Router skipped/unavailable; using safe no-code fallback.' : 'Fast path: Router skipped.',
      problem_type: math ? 'other' : 'general',
      difficulty: olympiad ? 'HARD' : 'MEDIUM',
      model: null,
      used: false
    };
  }

  const autoDeep = forcedDeep || olympiad || (deepCandidate && ['HARD', 'CHALLENGE'].includes(String(route.difficulty || '').toUpperCase()));
  const codeExecutionTools = route.needs_code === true && (autoDeep || deepCandidate);
  let candidate = null;
  let source = 'gemini';
  let audit = null;
  let repairCount = 0;
  ...
  try {
    candidate = await solveWithFallback({ message, subject, history, image, cas, deep: autoDeep, codeExecutionTools });
  } catch (e) {
  ...
  if (autoDeep && (math || olympiad)) {
    audit = await verify({ message, subject, image, cas, candidate });
    for (let i = 1; i <= MAX_REPAIR_ROUNDS && audit?.verdict === 'FAIL' && (Date.now() - started) < 40000; i++) {
      const repaired = await repair({ message, subject, image, cas, candidate, audit, attempt: i, codeExecutionTools });
      if (!repaired) break;
      candidate = repaired;
      repairCount++;
  ...
  return json(res, 200, {
    ...
    repairCount,
    repairSkippedByBudget: Boolean(audit?.verdict === 'FAIL' && (Date.now() - started) >= 40000),
    source,
