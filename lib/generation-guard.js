...
import { sanitizeDlpText, inspectSemanticConversation } from './api/_ai-input-guard.js';
import { installAiResponseGuard } from './api/_response-guard.js';
...
export async function protectGeneration(req,res,kind){
  ...
  if(req.body&&typeof req.body==='object'){
    // All user-controlled text that can enter a generation prompt is treated as untrusted.
    // DLP redaction runs first; a high-confidence semantic injection signal blocks the request.
    for (const field of ['documentText','userInstruction']) {
      if (typeof req.body[field] !== 'string') continue;
      const rawText = req.body[field];
      // Reject oversized prompt-bearing text before any provider call. The
      // generation handlers intentionally accept large multipart payloads,
      // but the extracted text itself has a bounded prompt budget.
      if (field === 'documentText' && rawText.length > 500_000) {
        res.status(413).json({ error: 'Nội dung tài liệu quá lớn.', code: 'document-text-too-large', requestId });
        return null;
      }
      const redacted = sanitizeDlpText(rawText);
      const semantic = inspectSemanticConversation(redacted.text, []);
      if (semantic.score >= 1) {
        await recordAgentSignal(req, 'generation-prompt-injection');
        res.status(422).json({ error: 'Nội dung tài liệu/yêu cầu chứa tín hiệu prompt injection bị chặn.', code: 'generation-prompt-injection', requestId });
        return null;
      }
      req.body[field] = redacted.text;
    }
  ...
  return ()=>{uninstallAiResponseGuard();if(originalJson)res.json=originalJson;if(originalEnd)res.end=originalEnd;};
}
